...
class Slider(Element):
    ...
    def __create_track_vao(self):
        ...
        """
        gl_x = (
            2 * ((self.x - (self.width / 2) -
                 (self.app.WIN_SIZE[0] / 2) - self.width / self.slices) / (self.app.WIN_SIZE[0])),
            2 * ((self.x + (self.width / 2) -
                 (self.app.WIN_SIZE[0] / 2) + self.width / self.slices) / (self.app.WIN_SIZE[0]))
        )
    ...
    def __create_handle_vao(self):
        ...
        handle_vertices = np.array([
            self.__to_gl_x(handle_x - self.width /
                           self.slices), self.__to_gl_y(self.y - self.height / 2), 0.0,
            self.__to_gl_x(handle_x + self.width /
                           self.slices), self.__to_gl_y(self.y - self.height / 2), 0.0,
            self.__to_gl_x(handle_x - self.width /
                           self.slices), self.__to_gl_y(self.y + self.height / 2), 0.0,
    ...
    def update_value(self, mouse_x):
        ...
        if self.is_dragging:
            new_value = self.min_value + \
                ...
                self.width * (self.max_value - self.min_value)
            self.current_slice = min(range(len(self.handle_values)), key=lambda i: abs(
                self.handle_values[i] - new_value))
            self.handle_vao = self.__create_handle_vao()
